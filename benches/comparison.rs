...
fn benchmark_hashing(c: &mut Criterion) {
    ...
    let _ = group.bench_function("sha256", |b| {
        b.iter(|| hashing::sha256_hash(black_box(&data)))
    });

    let _ = group.bench_function("blake3", |b| {
        b.iter(|| hashing::blake3_hash(black_box(&data)))
    });
...
fn benchmark_compression(c: &mut Criterion) {
    ...
    let _ = group.bench_function("zstd_level_3", |b| {
        b.iter(|| compression::compress_zstd(black_box(&data), Some(3)))
    });

    let _ = group.bench_function("lz4", |b| {
        b.iter(|| compression::compress_lz4(black_box(&data)))
    });
